...
在深入区块结构之前，我们先了解一个重要的数据结构：Merkle 树。它让我们能够高效地验证一笔交易是否在区块中。
...
- **纯指针式**：使用 `left` 和 `right` 字段保存子节点的对象引用，构建真正的树形结构
...
| 操作 | 复杂度 | 说明 |
|------|--------|------|
| 挖矿（找nonce） | O(16^d) | 需要大量尝试 |
| 验证（检查哈希） | O(1) | 只需一次哈希计算 |

这种不对称性确保了：
1. 任何人都能快速验证区块的有效性
2. 但要创建有效区块需要投入大量计算资源
3. 修改历史记录需要重新挖掘所有后续区块，成本极高
...
比特币的设计是优雅的。它用工作量证明实现了去中心化的共识，用区块链实现了防篡改的账本，用难度调整保证了系统的稳定性。通过自己动手实现这些组件，我们不仅理解了比特币的工作原理，也体会到了它设计的精妙之处。

到这里，我们已经实现了一个完整的比特币核心系统。在下一篇文章中，我们可能会探索更高级的话题，比如网络协议、共识算法的变体，或者智能合约等扩展功能。
